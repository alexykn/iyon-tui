...
## T15 authoritative completion record

**1. Scope statement.** Complete the transport-parity assessment, authoritative §93 matrix, §95 multi-edit gate, §96 wide/path cases, generic §99 trace, §90 phase visibility, §102 statistics, and §59 memory recheck for generated safe N-API versus the feature-gated direct-FFI oracle. No transport selection or cleanup is performed by this record.

**2. Commits and artifacts.** Stateful/fair workload and authoritative orchestrator: `bb2029b`; memory gate: `8911bcb`; fallback phase correction: `e1fdd93`; multi-edit runner: `701b680`; generic trace runner: `6efb3d9`; final common matrix artifact: `3a76f50`; final memory artifact: `7acdc10`; final multi-edit artifact: `80707ce`; final generic trace artifact: `f521acd`. The parity assessment is recorded in `PERF-12-T15-TRANSPORT-PARITY.md`; the published report is `PERF-12-T15-AUTHORITATIVE-REPORT.md`.

**3. Review findings and corrections.** The draft's per-sample pair factory rebuilt supposedly stable subtrees and therefore did not measure retained identity fairly; it now uses stateful scenarios with persistent shared subtrees. The draft had no complete process-isolated matrix runner; the final runner covers 311 cases per arm and alternates arm order by workload block. Large fallback cases previously emitted no phase samples; fallback host-commit timing is now recorded so all phase arrays align. Direct and safe retained/native source review found only physical lowering differences: both arms use the same semantic IR, retained DAG, leases, hints, derivations, PersistentSeq paths, materializer families, native host, layout, paint, cache, and recovery implementation. Direct ABI wrappers match all 57 canonical functions and remain feature-gated.

**4. Implementation summary.** The final authoritative harness runs fresh child processes, stages the default and `direct-ffi` artifacts separately, emits self-contained raw JSONL, compares structural deltas and screen output, reports median/p95/p99/bootstrap statistics, and writes a summary. Supplemental runners cover typed multi-edit transactions, wide axis/grid/path derivations, a generic history/stream/status/layout trace, and one-million-operation memory churn. No direct-FFI code was deleted, disabled, or hidden after measurement.

**5. Provenance.** Final harness source: `019a048b7c6fa8e6cc6c1e4f0e6635dc78c1b0b7`; raw artifacts were captured from clean committed follow-on artifact revisions listed above. Bun `1.4.0` / revision `34cbb9a40b4bd1bd767d134a7065e66c2432a676`; rustc `1.97.1 (8bab26f4f, aarch64-apple-darwin)`; schema BLAKE3 `5e7332e72b071e87f451f9710dd21d6d9f707277281abe50f2583dc3509c1745`.

**6. Gate evidence with actual counts/numbers.** The common matrix completed 311 cases per arm / 622 records with zero correctness or structural mismatches and zero phase-array length mismatches. The heterogeneous common-matrix geometric mean is N-API/direct = `1.0421` (N-API approximately 4.2% slower). Multi-edit cases at 2/8/32/64 edits all passed final screen parity; median ratios are approximately `1.049/1.055/1.050/1.035`. The generic trace produced matching first-screen output and 1,000 phase-visible samples per arm; medians are 18,791 ns N-API versus 15,750 ns direct. The memory gate completed one million operations per arm and converged after maintenance to 204 semantic entries, 204 native-ref slots, 1 leased slot, and 203 unleased live slots for both arms. These are technical comparison results, not a transport adoption decision.

**7. Status.** **T15 COMPLETE — owner adoption pending.** The authoritative evidence and report are committed. Both generated safe N-API and feature-gated direct FFI remain buildable and retained until the repository owner explicitly decides what to adopt or remove. T16 remains blocked and no cleanup is claimed.