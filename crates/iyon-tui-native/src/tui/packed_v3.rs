...
use super::{
    ...
    OVERFLOW_ELLIPSIS, OVERFLOW_FOOTER, OVERFLOW_NONE, PACKED_V3_COLD_CLOSURE,
    PACKED_V3_DEF_GRID_CELL_BRANCH, PACKED_V3_DEF_GRID_CELL_LEAF, PACKED_V3_DEF_SEQ_BRANCH,
    PACKED_V3_DEF_SEQ_LEAF, PACKED_V3_DEF_VIEW_FULL, PACKED_V3_HAS_BYTE_LANE,
    PACKED_V3_HAS_STRING_LANE, PACKED_V3_OP_RENDER, PACKED_V3_OP_RENDER_FOREST,
    PACKED_V3_PATCH_ALIGN, PACKED_V3_PATCH_AXIS, PACKED_V3_PATCH_DECORATION, PACKED_V3_PATCH_GAP,
    PACKED_V3_PATCH_GRID, PACKED_V3_PATCH_GRID_CELLS, PACKED_V3_PATCH_HEIGHT,
    PACKED_V3_PATCH_MAX_HEIGHT, PACKED_V3_PATCH_MAX_WIDTH, PACKED_V3_PATCH_MIN_HEIGHT,
    PACKED_V3_PATCH_MIN_WIDTH, PACKED_V3_PATCH_PADDING, PACKED_V3_PATCH_SEQUENCE,
    PACKED_V3_PATCH_TEXT, PACKED_V3_PATCH_VIEW, PACKED_V3_PATCH_WIDTH, PACKED_V3_PATCH_WRAP,
    PACKED_V3_PROTOCOL_VERSION, PACKED_V3_RESET_GENERATION, PACKED_V3_SEQ_BRANCH_FACTOR,
    PACKED_V3_SEQ_COLUMN, PACKED_V3_SEQ_ROW, PACKED_V3_WIRE_LOCAL_BIT, PACKED_VIEW_MAGIC,
...
const KNOWN_FLAGS: u32 = PACKED_V3_RESET_GENERATION
    | PACKED_V3_COLD_CLOSURE
    | PACKED_V3_HAS_BYTE_LANE
    | PACKED_V3_HAS_STRING_LANE;
...
fn cache_miss(reference: u32) -> napi::Error {
    crate::NativeError::coded(
        ...
        format!("packed V3 reference {reference} is not retained"),
    )
}
...
enum PackedSlot {
    ...
    Sequence(Weak<V3Sequence>),
    GridSequence(Weak<V3GridSequence>),
}
...
impl PackedSlotTable {
    ...
    fn sequence(&self, reference: u32) -> Option<Arc<V3Sequence>> {
        match self.get(reference)? {
    ...
    fn grid_sequence(&self, reference: u32) -> Option<Arc<V3GridSequence>> {
        match self.get(reference)? {
...
// See V3Sequence: descendant sequence Arcs are lifetime anchors for weak
...
enum V3GridSequence {
    Leaf {
        aggregate: u32,
        items: Arc<[V3GridCell]>,
        retained: RetainedGridCells,
    },
    Branch {
        height: u32,
        aggregate: u32,
        children: Arc<[Arc<V3GridSequence>]>,
        sizes: Arc<[u32]>,
        retained: RetainedGridCells,
    },
}
...
pub fn resolve_ref(generation: i64, packed_ref: i64, cache: ViewRuntimeHandle) -> Result<View> {
    ...
    inc(iyon_tui::perf::Counter::NapiV3PersistentRefMisses);
...
impl<'a> V3Transaction<'a> {
    fn new(
        ...
        if words[0] != PACKED_VIEW_MAGIC {
            return Err(invalid("unknown packed V3 magic"));
        }
        if words[1] != PACKED_V3_PROTOCOL_VERSION {
            return Err(invalid("unsupported packed V3 protocol version"));
        }
        if words[2] != VIEW_BRIDGE_SCHEMA_VERSION {
            return Err(invalid("packed V3 bridge schema mismatch"));
        }
        let generation = words[3];
        let flags = words[4];
        if flags & !KNOWN_FLAGS != 0 {
            return Err(invalid("packed V3 flags contain unknown mandatory bits"));
        }
        let has_byte_lane = flags & PACKED_V3_HAS_BYTE_LANE != 0;
        let has_string_lane = flags & PACKED_V3_HAS_STRING_LANE != 0;
        if has_byte_lane == has_string_lane {
            return Err(invalid("packed V3 requires exactly one string lane"));
        }
        if has_byte_lane && strings.is_some() {
            return Err(invalid(
                "packed V3 string lane arguments do not match byte lane",
            ));
        }
        if has_string_lane && strings.is_none() {
            return Err(invalid("packed V3 string lane arguments are missing"));
        }
        let used_words =
            usize::try_from(words[5]).map_err(|_| invalid("packed V3 word count is invalid"))?;
        if used_words < 10 || used_words > words.len() {
            return Err(invalid("packed V3 word count is out of bounds"));
        }
        let used_bytes =
            usize::try_from(words[6]).map_err(|_| invalid("packed V3 byte count is invalid"))?;
        if used_bytes > bytes.len() {
            return Err(invalid("packed V3 byte count is out of bounds"));
        }
        if has_string_lane && used_bytes != 0 {
            return Err(invalid("packed V3 string lane must not carry byte data"));
        }
        if words[9] != 0 {
            return Err(invalid("packed V3 reserved header word is nonzero"));
        }
        let definition_count = usize::try_from(words[7])
            .map_err(|_| invalid("packed V3 definition count is invalid"))?;
        let operation_count = usize::try_from(words[8])
            .map_err(|_| invalid("packed V3 operation count is invalid"))?;
        if operation_count != 1 {
            return Err(invalid("packed V3 requires exactly one operation"));
        }
        let cold = flags & PACKED_V3_COLD_CLOSURE != 0;
        ...
        if flags & PACKED_V3_RESET_GENERATION != 0 {
            let expected = cache_generation
                .checked_add(1)
                .ok_or_else(|| invalid("packed V3 generation exhausted"))?;
            if generation != expected {
                return Err(invalid(
                    "packed V3 reset generation is not the next generation",
                ));
            }
        }
        Ok(Self {
            ...
            strings: strings.map(|values| values.into_iter().map(Some).collect()),
            cursor: 10,
    ...
    fn decode_definitions(&mut self) -> Result<()> {
        for _ in 0..self.definition_count {
            ...
            if record_words < 3 || start.checked_add(record_words).is_none() {
                return Err(invalid("packed V3 definition length is invalid"));
            }
            let end = start + record_words;
            if end > self.words.len() {
                return Err(invalid("packed V3 definition exceeds transaction"));
            }
            let raw_reference = self.word("definition ref")?;
            let reference = persistent_ref(raw_reference as i64)?;
            if !self.refs.insert(reference) {
                return Err(invalid("packed V3 duplicate persistent ref"));
            }
            let value = match tag {
                PACKED_V3_DEF_VIEW_FULL => {
                    inc(iyon_tui::perf::Counter::NapiV3FullViewsBuilt);
                    self.decode_full(end)?
                }
                PACKED_V3_PATCH_VIEW => {
                    inc(iyon_tui::perf::Counter::NapiV3ViewsPatched);
                    self.decode_patch(end)?
                }
                PACKED_V3_DEF_SEQ_LEAF => {
                    inc(iyon_tui::perf::Counter::NapiV3SeqNodesBuilt);
                    self.decode_sequence_leaf(end)?
                }
                PACKED_V3_DEF_SEQ_BRANCH => {
                    inc(iyon_tui::perf::Counter::NapiV3SeqNodesBuilt);
                    self.decode_sequence_branch(end)?
                }
                PACKED_V3_DEF_GRID_CELL_LEAF => {
                    inc(iyon_tui::perf::Counter::NapiV3SeqNodesBuilt);
                ...
                _ => return Err(invalid(format!("unknown packed V3 definition tag {tag}"))),
            };
            if self.cursor != end {
                return Err(invalid(
                    "packed V3 definition length does not match payload",
                ));
            }
    ...
    fn decode_operation(&mut self) -> Result<View> {
        ...
        let end = start
            .checked_add(length)
            .ok_or_else(|| invalid("packed V3 operation length overflow"))?;
        if end > self.words.len() || length < 3 {
            return Err(invalid("packed V3 operation is out of bounds"));
        }
        let root_count = self.count("operation root count")?;
        if tag == PACKED_V3_OP_RENDER && root_count != 1 {
            return Err(invalid(
                "packed V3 render operation requires exactly one root",
            ));
        }
        if tag == PACKED_V3_OP_RENDER_FOREST && root_count == 0 {
            return Err(invalid(
                "packed V3 forest operation requires at least one root",
            ));
        }
        if tag != PACKED_V3_OP_RENDER && tag != PACKED_V3_OP_RENDER_FOREST {
            return Err(invalid("unknown packed V3 operation"));
        }
        if tag == PACKED_V3_OP_RENDER_FOREST && root_count != 1 {
            return Err(invalid(
                "packed V3 forest operation requires a forest-capable host boundary",
            ));
        }
        ...
        if self.cursor != end || end != self.words.len() {
            return Err(invalid("packed V3 operation has trailing words"));
        }
        roots
            .into_iter()
            .next()
            .ok_or_else(|| invalid("packed V3 render requires a root"))
    }
        ...
        let view = match self.word("view kind")? {
            ...
            VIEW_KIND_ROW | VIEW_KIND_COLUMN => match self.decode_axis()? {
                ...
                Staged::Sequence(_) | Staged::GridSequence(_) => {
                    return Err(invalid("packed V3 axis did not decode to a View"));
                }
            },
            ...
            VIEW_KIND_GRID => match self.decode_grid()? {
                Staged::View { view, .. } => view,
                Staged::Sequence(_) | Staged::GridSequence(_) => {
                    return Err(invalid("packed V3 grid did not decode to a View"));
                }
            },
            VIEW_KIND_CONTAINER => {
            ...
            other => return Err(invalid(format!("unknown packed V3 View kind {other}"))),
        };
    ...
    fn decode_patch(&mut self, _end: usize) -> Result<Staged> {
        ...
        let view = match kind {
            PACKED_V3_PATCH_TEXT => {
                if mask == 0 || mask & !(PACKED_V3_PATCH_WRAP | PACKED_V3_PATCH_ALIGN) != 0 {
                    return Err(invalid("packed V3 text patch mask is invalid"));
                }
                let wrap = if mask & PACKED_V3_PATCH_WRAP != 0 {
                    Some(decode_wrap(self.word("patch wrap")?)?)
                } else {
                    None
                };
                let align = if mask & PACKED_V3_PATCH_ALIGN != 0 {
                ...
                let (base_wrap, base_align) = base
                    ...
                    .ok_or_else(|| invalid("packed V3 text patch base is not text"))?;
                if wrap.is_none_or(|value| value == base_wrap)
                    && align.is_none_or(|value| value == base_align)
                {
                    return Err(invalid("packed V3 text patch is unchanged"));
                }
                base.with_text_layout_patch(wrap, align)
            }
            PACKED_V3_PATCH_DECORATION => {
                if mask == 0
                    || mask
                        & !(PACKED_V3_PATCH_PADDING
                        ...
                        != 0
                {
                    return Err(invalid("packed V3 decoration patch mask is invalid"));
                }
                let padding = if mask & PACKED_V3_PATCH_PADDING != 0 {
                    Some(Insets::new(
                ...
                let width = if mask & PACKED_V3_PATCH_WIDTH != 0 {
                    Some(match self.word("patch width")? {
                        1 => RetainedSizeRule::Fit,
                        2 => RetainedSizeRule::Fill,
                        _ => return Err(invalid("packed V3 patch width rule is invalid")),
                    })
                } else {
                    None
                };
                let height = if mask & PACKED_V3_PATCH_HEIGHT != 0 {
                    Some(match self.word("patch height")? {
                        1 => RetainedSizeRule::Fit,
                        2 => RetainedSizeRule::Fill,
                        _ => return Err(invalid("packed V3 patch height rule is invalid")),
                    })
                } else {
                    None
                };
                let min_width = (mask & PACKED_V3_PATCH_MIN_WIDTH != 0)
                    .then(|| self.u16("patch minWidth"))
                    .transpose()?;
                let max_width = (mask & PACKED_V3_PATCH_MAX_WIDTH != 0)
                    .then(|| self.u16("patch maxWidth"))
                    .transpose()?;
                let min_height = (mask & PACKED_V3_PATCH_MIN_HEIGHT != 0)
                    .then(|| self.u16("patch minHeight"))
                    .transpose()?;
                let max_height = (mask & PACKED_V3_PATCH_MAX_HEIGHT != 0)
            ...
            PACKED_V3_PATCH_GRID => {
                if mask != PACKED_V3_PATCH_GRID_CELLS {
                    return Err(invalid("packed V3 grid patch mask is invalid"));
                }
            ...
            PACKED_V3_PATCH_AXIS => {
                if mask & !(PACKED_V3_PATCH_GAP | PACKED_V3_PATCH_SEQUENCE) != 0
                    || mask & PACKED_V3_PATCH_SEQUENCE == 0
                {
                    return Err(invalid("packed V3 axis patch mask is invalid"));
                }
                let horizontal = base
                    .retained_axis_horizontal()
                    .ok_or_else(|| invalid("packed V3 axis patch base is not an axis"))?;
                let sequence = self.resolve_next_sequence("axis patch sequence")?;
                if sequence.kind()
                    != if horizontal {
                        PACKED_V3_SEQ_ROW
                    } else {
                        PACKED_V3_SEQ_COLUMN
                    }
                {
                    return Err(invalid("packed V3 axis patch sequence kind is invalid"));
                }
                let gap = if mask & PACKED_V3_PATCH_GAP != 0 {
                    self.u16("axis patch gap")?
                } else {
                    base.retained_axis_gap()
                        .ok_or_else(|| invalid("packed V3 axis patch base gap is unavailable"))?
                };
                base.patch_retained_axis(sequence.retained_axis(), gap)
                    .retain_transport_payload(sequence.clone())
            }
            _ => return Err(invalid("unknown packed V3 patch kind")),
        };
        Ok(Staged::View { node_id, view })
    }
    ...
    fn resolve_next_sequence(&mut self, name: &str) -> Result<Arc<V3Sequence>> {
        let reference = self.word(name)?;
        self.resolve_sequence(reference)
    }

    fn resolve_next_grid_sequence(&mut self, name: &str) -> Result<Arc<V3GridSequence>> {
        let reference = self.word(name)?;
    ...
    fn decode_sequence_leaf(&mut self, _end: usize) -> Result<Staged> {
        ...
        if sequence_kind != PACKED_V3_SEQ_ROW && sequence_kind != PACKED_V3_SEQ_COLUMN {
            return Err(invalid("packed V3 sequence leaf kind is invalid"));
        }
        ...
        if count > PACKED_V3_SEQ_BRANCH_FACTOR as usize {
            return Err(invalid("packed V3 sequence leaf exceeds branch factor"));
        }
        ...
        let horizontal = sequence_kind == PACKED_V3_SEQ_ROW;
        ...
        if aggregate != u32::from(retained.aggregate_flags()) {
            return Err(invalid("packed V3 sequence leaf aggregate is invalid"));
        }
        Ok(Staged::Sequence(Arc::new(V3Sequence::Leaf {
            kind: sequence_kind,
            aggregate,
            items: items.into(),
            retained,
    ...
    fn decode_sequence_branch(&mut self, _end: usize) -> Result<Staged> {
        ...
        if kind != PACKED_V3_SEQ_ROW && kind != PACKED_V3_SEQ_COLUMN {
            return Err(invalid("packed V3 sequence branch kind is invalid"));
        }
        ...
        if count == 0 || count > PACKED_V3_SEQ_BRANCH_FACTOR as usize {
            return Err(invalid("packed V3 sequence branch count is invalid"));
        }
        let aggregate = self.word("sequence aggregate")?;
        ...
        for _ in 0..count {
            ...
            if size <= previous {
                return Err(invalid("packed V3 sequence cumulative sizes are invalid"));
            }
            previous = size;
            sizes.push(size);
        ...
        .map_err(invalid)?;
        Ok(Staged::Sequence(Arc::new(V3Sequence::Branch {
            kind,
            height,
            aggregate,
    ...
    fn decode_grid_sequence_leaf(&mut self, _end: usize) -> Result<Staged> {
        ...
        if count > PACKED_V3_SEQ_BRANCH_FACTOR as usize {
            return Err(invalid(
                "packed V3 grid sequence leaf exceeds branch factor",
            ));
        }
        ...
        if aggregate != u32::from(retained.aggregate_flags()) {
            return Err(invalid("packed V3 grid sequence leaf aggregate is invalid"));
        }
        Ok(Staged::GridSequence(Arc::new(V3GridSequence::Leaf {
            aggregate,
            items: items.into(),
            retained,
        })))
    ...
    fn decode_grid_sequence_branch(&mut self, _end: usize) -> Result<Staged> {
        ...
        if count == 0 || count > PACKED_V3_SEQ_BRANCH_FACTOR as usize {
            return Err(invalid("packed V3 grid sequence branch count is invalid"));
        }
        let aggregate = self.word("grid sequence aggregate")?;
        let mut children = Vec::with_capacity(count);
        let mut sizes = Vec::with_capacity(count);
        ...
        for _ in 0..count {
            ...
            if size <= previous {
                return Err(invalid(
                    "packed V3 grid sequence cumulative sizes are invalid",
                ));
            }
            previous = size;
            sizes.push(size);
        ...
        {
            return Err(invalid(
                "packed V3 grid sequence child height or size is invalid",
            ));
        }
        if sizes.last().copied().unwrap_or(0)
            != children.iter().map(|child| child.length()).sum::<u32>()
        {
            return Err(invalid(
                "packed V3 grid sequence length aggregate is invalid",
            ));
        }
        ...
        if aggregate != calculated {
            return Err(invalid(
                "packed V3 grid sequence branch aggregate is invalid",
            ));
        }
        let retained =
            RetainedGridCells::branch(children.iter().map(|child| child.retained()).collect())
                .map_err(invalid)?;
        Ok(Staged::GridSequence(Arc::new(V3GridSequence::Branch {
            height,
            aggregate,
            children: children.into(),
            sizes: sizes.into(),
    ...
    fn decode_axis(&mut self) -> Result<Staged> {
        ...
        if sequence.kind() != PACKED_V3_SEQ_ROW && sequence.kind() != PACKED_V3_SEQ_COLUMN {
            return Err(invalid("packed V3 axis sequence kind is invalid"));
        }
        let view = sequence
            .retained_axis()
            .into_view(gap)
    ...
    fn decode_overflow(&mut self) -> Result<iyon_tui::OverflowIndicator> {
        match self.word("overflow kind")? {
            ...
            _ => Err(invalid("invalid packed V3 overflow kind")),
        }
    }
    ...
    fn decode_decoration(&mut self, view: View) -> Result<View> {
        ...
        if flags & !0x0fff != 0 || flags & 16 == 0 {
            return Err(invalid("packed V3 decoration style is required"));
        }
        ...
        if flags & 32 != 0 {
            ...
            for _ in 0..count {
                ...
                if key.is_empty() || value.is_empty() {
                    return Err(invalid(
                        "packed V3 style state key and value cannot be empty",
                    ));
                }
        ...
        let width = if flags & 64 != 0 {
            Some(match self.word("width rule")? {
                1 => RetainedSizeRule::Fit,
                2 => RetainedSizeRule::Fill,
                _ => return Err(invalid("packed V3 width rule is invalid")),
            })
        } else {
            None
        };
        let height = if flags & 128 != 0 {
            Some(match self.word("height rule")? {
                1 => RetainedSizeRule::Fit,
                2 => RetainedSizeRule::Fill,
                _ => return Err(invalid("packed V3 height rule is invalid")),
            })
        } else {
            None
        };
    ...
    fn decode_style(&mut self) -> Result<StyleRef> {
        ...
        if flags & !(1 | 2 | 4) != 0 {
            return Err(invalid("packed V3 style flags are invalid"));
        }
        ...
        if present & !STYLE_ALL != 0 || truth & !present != 0 {
            return Err(invalid("packed V3 style masks are invalid"));
        }
    ...
    fn color(&mut self) -> Result<ColorSpec> {
        match self.word("color kind")? {
            ...
            _ => Err(invalid("invalid packed V3 color")),
        }
    }
    ...
    fn border(&mut self) -> Result<BorderSpec> {
        ...
        if flags & !0x0f != 0 {
            return Err(invalid("packed V3 border flags are invalid"));
        }
        ...
        let mut spec = match style.unwrap_or(1) {
            ...
            3 => BorderSpec::double(),
            _ => return Err(invalid("invalid packed V3 border style")),
        };
        ...
        if edges.is_some_and(|value| value != 1 && value != 2) {
            return Err(invalid("invalid packed V3 border edges"));
        }
    ...
    fn grid_track(&mut self) -> Result<GridTrack> {
        ...
        match kind {
            ...
            _ => Err(invalid("invalid packed V3 grid track")),
        }
    }
    ...
    fn resolve_view(&self, word: u32) -> Result<View> {
        match self.resolve(word)? {
            ...
            Staged::Sequence(_) | Staged::GridSequence(_) => {
                Err(invalid("packed V3 expected a View reference"))
            }
        }
    }
    fn resolve_sequence(&self, word: u32) -> Result<Arc<V3Sequence>> {
        match self.resolve(word)? {
            Staged::Sequence(sequence) => Ok(sequence),
            _ => Err(invalid("packed V3 expected an axis sequence reference")),
        }
    }

    fn resolve_grid_sequence(&self, word: u32) -> Result<Arc<V3GridSequence>> {
        match self.resolve(word)? {
            Staged::GridSequence(sequence) => Ok(sequence),
            _ => Err(invalid("packed V3 expected a grid sequence reference")),
        }
    }

    fn resolve(&self, word: u32) -> Result<Staged> {
        if word & PACKED_V3_WIRE_LOCAL_BIT != 0 {
            let index = (word & !PACKED_V3_WIRE_LOCAL_BIT) as usize;
            inc(iyon_tui::perf::Counter::NapiV3LocalRefResolves);
            return self
                .definitions
                .get(index)
                .cloned()
                .ok_or_else(|| invalid("packed V3 local reference is forward or out of range"));
        }
        if self.cold {
            return Err(invalid(
                "packed V3 cold closure contains a persistent reference",
            ));
        }
        ...
        if let Some(sequence) = self.slots.sequence(reference) {
            inc(iyon_tui::perf::Counter::NapiV3SeqNodesReused);
            inc(iyon_tui::perf::Counter::NapiV3PersistentRefUpgrades);
        ...
        for (_, value) in &self.staged_refs {
            ...
            if !transaction_node_ids.insert(*node_id) {
                return Err(invalid(format!(
                    "packed V3 duplicate NodeId {node_id} in transaction",
                )));
            }
            ...
            {
                return Err(invalid(format!(
                    "packed V3 NodeId {node_id} changed semantic identity",
                )));
            }
        }

        if self.cold {
            cache.packed_v3.slots.reset();
            cache.packed_v3.generation = self.generation;
        }
        for (reference, value) in self.staged_refs.drain(..) {
            inc(iyon_tui::perf::Counter::NapiV3CachePublications);
            match value {
    ...
    fn string(&mut self) -> Result<String> {
        if self.strings.is_some() {
            let index = self.word("string index")? as usize;
            let reserved = self.word("string lane reserved word")?;
            if reserved != 0 {
                return Err(invalid("packed V3 string lane reserved word is nonzero"));
            }
            let strings = self
                .strings
                .as_mut()
                .expect("string lane checked at construction");
            let value = strings
                .get_mut(index)
                .ok_or_else(|| invalid("packed V3 string index is out of bounds"))?
                .take()
                .ok_or_else(|| invalid("packed V3 string index is reused"))?;
            return Ok(value);
        }
        let offset = self.word("string offset")? as usize;
        let length = self.word("string length")? as usize;
        let end = offset
            .checked_add(length)
            .ok_or_else(|| invalid("packed V3 string range overflow"))?;
        let bytes = self
            .bytes
            .get(offset..end)
            .ok_or_else(|| invalid("packed V3 string range is out of bounds"))?;
        inc(iyon_tui::perf::Counter::NapiV3Utf8Validations);
        add(iyon_tui::perf::Counter::NapiV3BytesRead, length);
        String::from_utf8(bytes.to_vec())
            .map_err(|_| invalid("packed V3 string is not valid UTF-8"))
    }
    fn node_id(&mut self) -> Result<u64> {
        ...
        if value == 0 || value > MAX_SAFE_INTEGER {
            return Err(invalid("packed V3 NodeId is not a positive safe integer"));
        }
        Ok(value)
    }
    ...
    fn word(&mut self, name: &str) -> Result<u32> {
        let value = *self
            .words
            .get(self.cursor)
            .ok_or_else(|| invalid(format!("packed V3 transaction is missing {name}")))?;
        self.cursor += 1;
        inc(iyon_tui::perf::Counter::NapiV3WordsRead);
        Ok(value)
    }
}
...
fn validate_sequence_child(kind: u32, size: u16, max_rows: u16, sequence_kind: u32) -> Result<()> {
    let valid = match kind {
        ...
        LAYOUT_CHILD_FLEX_MAX | LAYOUT_CHILD_CONTENT_MAX => {
            sequence_kind == PACKED_V3_SEQ_COLUMN && size == 0
        }
        _ => false,
    };
    if valid {
        ...
        Err(invalid("packed V3 sequence child fields are invalid"))
    }
}

fn persistent_ref(value: i64) -> Result<u32> {
    let value =
        u32::try_from(value).map_err(|_| invalid("packed V3 persistent ref must fit u32"))?;
    if value == 0 || value >= PACKED_V3_WIRE_LOCAL_BIT {
        return Err(invalid("packed V3 persistent ref is out of range"));
    }
    Ok(value)
}
fn decode_wrap(value: u32) -> Result<WrapMode> {
    match value {
        ...
        _ => Err(invalid("invalid packed V3 wrap mode")),
    }
}
fn decode_horizontal_align(value: u32) -> Result<HorizontalAlign> {
    match value {
        ...
        _ => Err(invalid("invalid packed V3 horizontal alignment")),
    }
}
fn decode_vertical_align(value: u32) -> Result<VerticalAlign> {
    match value {
        ...
        _ => Err(invalid("invalid packed V3 vertical alignment")),
    }
}
