...
pub unsafe extern "Rust" fn view_content_host_create_impl(
    ...
    let _ = node_id_low;
    let _ = node_id_high;
    let _ = content_port_id_low;
    let _ = content_port_id_high;
    0x105
}
...
fn generated_function_count_is_stable() {
    assert_eq!(generated::FUNCTION_COUNT, 60);
}
...
fn generated_wrappers_reject_invalid_inputs_and_delegate() {
    ...
    assert_eq!(
        unsafe { generated_exports::invoke_iyon_view_state_attach_v1(runtime_ptr, 1, 1, 0, 1, 0) },
        0x104
    );
    assert_eq!(
        unsafe {
            generated_exports::invoke_iyon_view_content_host_create_v1(runtime_ptr, 1, 0, 1, 0)
        },
        0x105
    );
    assert_eq!(
    ...
    assert_eq!(
        unsafe { generated_exports::invoke_iyon_view_state_attach_v1(runtime_ptr, 0, 1, 1, 1, 0) },
        0x8000_0001
    );
    assert_eq!(
