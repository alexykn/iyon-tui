/**
 * Shared semantic-tree fixtures for PERF-11v4/PERF-12 probes and tests.
 *
 * PERF-12 T4 note: this is the View-only descendant of the removed
 * PERF-11v4 fixtures module. The Perf7v2View mirror arm is gone (the
 * candidate won and became production), and the ruled-out packed-transport
 * statics are no longer used: WIDE_PARENT_* modes rebuild the edited parent
 * eagerly through the public semantic API, which is exactly what the
...
function factory<T extends View>(): Factory<T> {
  const viewFactory: Omit<Factory<View>, never> = factoryLiteral();
  return viewFactory as unknown as Factory<T>;
}

function factoryLiteral(): Factory<View> {
...
function plain<T extends View>(f: Factory<T>, n: number, prefix: string): T { return f.vertical((column) => { for (let i = 0; i < Math.max(1, n - 1); i += 1) column.child(f.text(`${prefix}-${i}`)); }); }
function styled<T extends View>(f: Factory<T>, n: number): T { return f.vertical((column) => { for (let i = 0; i < Math.max(1, Math.min(n - 1, 256)); i += 1) column.child(f.textAlign(f.wrap(f.styledText([TextSpan.plain("prefix "), TextSpan.styled(`styled-${i}`, Style.new().bold().foreground(i % 2 ? "yellow" : "cyan").attribute("italic", i % 3 === 0)), TextSpan.plain(" suffix")]), i % 2 === 0 ? "wordThenGrapheme" : "grapheme"), i % 3 === 0 ? "center" : "start")); }); }
function rows<T extends View>(f: Factory<T>, n: number): T { return f.vertical((column) => { for (let i = 0; i < Math.max(1, Math.ceil((n - 1) / 4)); i += 1) column.child(f.horizontal((row) => { row.gap(i % 3); row.child(f.text(`a-${i}`)); row.fixed(4, f.text(`b-${i}`)); row.flex(f.text(`c-${i}`)); })); }); }
function tracksTree<T extends View>(f: Factory<T>, n: number): T { return f.vertical((column) => { for (let i = 0; i < Math.max(1, n - 1); i += 1) { const child = f.text(`track-${i}`); if (i % 4 === 0) column.fixed(8, child); else if (i % 4 === 1) column.flex(child); else if (i % 4 === 2) column.flexMax(3, child); else column.contentMax(2, child); } }); }
function gridTree<T extends View>(f: Factory<T>, n: number): T { return f.grid((grid) => { grid.columns(tracks); grid.columnGap(1); grid.rowGap(1); for (let i = 0; i < Math.max(1, Math.ceil((n - 1) / 4)); i += 1) grid.rowWith(i % 2 ? tracks[4]! : tracks[0]!, (row) => { row.cellWith({ columnSpan: 1, horizontalAlign: "start" }, f.text(`grid-a-${i}`)); row.cellWith({ columnSpan: 2, verticalAlign: "center" }, f.text(`grid-b-${i}`)); }); }); }
function decorations<T extends View>(f: Factory<T>, n: number): T { return f.vertical((column) => { for (let i = 0; i < Math.max(1, Math.min(n - 1, 128)); i += 1) { let child = f.text(`decorated-${i}`); child = f.padding(i % 2 ? 2 : 1, child); child = f.foreground(i % 2 ? { type: "ansi", value: 33 } : "green", child); child = f.background("theme:surface", child); child = f.border(child, i % 3 === 0 ? customBorder : { style: "plain", edges: "topBottom", color: "cyan" }); child = f.style(child, Style.new().bold().attribute("dim", i % 2 === 0)); child = f.styleState(child, "phase", i % 2 === 0 ? "a" : "b"); child = f.minMax(child); column.child(child); } }); }
function mixed<T extends View>(f: Factory<T>, n: number): T { const each = Math.max(2, Math.floor(n / 5)); return f.vertical((column) => { column.child(plain(f, each, "mixed-text")); column.child(styled(f, each)); column.child(rows(f, each)); column.child(decorations(f, each)); column.child(f.diff(n)); }); }
function workload<T extends View>(f: Factory<T>, name: ComparisonWorkload, n: number, suffix: string): T { switch (name) { case "plain_text_column": return plain(f, n, suffix); case "styled_span_heavy": return styled(f, n); case "row_heavy": return rows(f, n); case "column_track_heavy": return tracksTree(f, n); case "grid_heavy": return gridTree(f, n); case "decoration_heavy": return decorations(f, n); case "diff_heavy": case "large_diff_one_hunk_edit": return f.diff(n); case "component_heavy": return f.vertical((column) => { column.child(f.component(componentId)); column.child(f.text("component-tail")); }); case "mixed_realistic": return mixed(f, n); case "long_text_wrap_only": case "long_text_one_span_edit": return f.text("x".repeat(Math.min(8192, Math.max(64, n * 8)))); case "large_decoration_only_change": return f.padding(1, f.text(`${suffix}-decoration`)); } }

function deepDepth(size: number): number { return size >= 10_000 ? 128 : size >= 2_000 ? 64 : size >= 200 ? 16 : 4; }

/** Wide-parent edit: eagerly rebuilds the edited parent through the semantic API. */
function buildWideNext<T extends View>(base: T, children: readonly T[], mode: ComparisonMode, index: number, make: (values: readonly T[]) => T): T {
  void base;
  const position = mode === "WIDE_PARENT_ONE_EDIT" ? index % children.length : Math.floor(children.length / 2);
  const next = [...children];
  if (mode === "WIDE_PARENT_ONE_EDIT") next[position] = View.text(`edited-${index}`) as T;
  if (mode === "WIDE_PARENT_INSERT") next.splice(position, 0, View.text(`inserted-${index}`) as T);
  if (mode === "WIDE_PARENT_REMOVE") next.splice(position, 1);
  return make(next);
}

export function buildComparisonPair<T extends View>(c: ComparisonCase, index: number): ComparisonPair<T> {
  const f = factory<T>(); const base = workload(f, c.workload, c.size, `base-${index}`);
...
export function prepareComparisonCase<T extends View>(c: ComparisonCase): PreparedComparisonCase<T> {
  ...
  if (c.mode === "WIDE_PARENT_ONE_EDIT" || c.mode === "WIDE_PARENT_INSERT" || c.mode === "WIDE_PARENT_REMOVE") { const children = Array.from({ length: Math.max(1, c.size) }, (_, i) => f.text(`wide-${i}`)); const make = (values: readonly T[]) => f.vertical((column) => { for (const value of values) column.child(value); }); const wideBase = make(children); return { base: wideBase, cold: false, next: (index) => buildWideNext(wideBase, children, c.mode, index, make) }; }
...
export function randomizedTree(seed: number, depth = 4): View {
  const f = factory<View>();
  const random = deterministicRandom(seed);
  const shared = f.styledText([TextSpan.styled(`共享-${seed}-λ`, Style.new().bold().foreground("cyan"))]);
  const build = (level: number): View => {
    if (level === 0) {
...
export function buildTracePair<T extends View>(index: number): ComparisonPair<T> & { readonly category: TraceCategory } { const slot = index % 100; const category: TraceCategory = slot < 55 ? "stream_append" : slot < 70 ? "no_view_change" : slot < 80 ? "view_replace" : slot < 88 ? "layout_change" : slot < 93 ? "component_update" : slot < 97 ? "history_update" : "structural_update"; const mode: ComparisonMode = category === "stream_append" ? "SHARED_PATH" : category === "no_view_change" ? "IDENTICAL_IDENTITY" : category === "view_replace" ? "REBUILT_EQUIVALENT" : category === "layout_change" ? "TEXT_METADATA_PATCH" : category === "component_update" ? "SHARED_PATH" : category === "history_update" ? "LARGE_SHARED_SUBTREE_CUTOFF" : "SHARED_DEEP"; return { ...buildComparisonPair<T>({ workload: "mixed_realistic", size: 200, mode, label: `trace/${category}/${index}` }, index), category }; }

export function fullSchemaPair<T extends View>(): ComparisonPair<T> {
  const f = factory<T>();
  const allAttributes = Style.new().bold().dim().italic().underline().reversed().strikethrough();
  const base = f.vertical((column) => {
    column.gap(1);
    column.child(f.wrap(f.text("文本🙂 é"), "grapheme"));
...
export function randomizedRetainedPair<T extends View>(seed: number, depth = 4): ComparisonPair<T> {
  const f = factory<View>();
  const shared = f.styledText([TextSpan.styled(`retained-${seed}-λ`, Style.new().bold().foreground("cyan"))]);
  const base = f.vertical((column) => { column.child(shared); column.child(randomizedTree(seed, depth)); }) as T;
  const next = f.vertical((column) => { column.child(shared); column.child(f.styledText([TextSpan.styled(`retained-edit-${seed}-終`, Style.new().underline())])); }) as T;
  return { base, next, cold: false };
}
