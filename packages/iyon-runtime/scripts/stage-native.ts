...
if (Bun.argv.includes("--standalone")) {
  const { iyonVirtualModulePlugin } = await import("../src/virtual-modules.ts");
  const output = new URL("iyon-smoke", repositoryDirectory).pathname;
  ...
  if (!build.success) {
    throw new Error(
      `standalone Bun compilation failed:\n${build.logs.map((log) => log.message).join("\n")}`,
    );
  }
  console.log(`built standalone ${output}`);
}
