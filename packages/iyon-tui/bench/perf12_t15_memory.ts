...
const CASE_RUNNER = fileURLToPath(new URL("./perf12_t15_memory_case.ts", import.meta.url));
...
async function main(): Promise<void> {
  ...
  if (status !== "") throw new Error("authoritative memory gate requires a clean committed checkout");
  const sourceSha = checked(["git", "rev-parse", "HEAD"]);
  const output = fileURLToPath(new URL(`./PERF-12-T15-memory-${sourceSha.slice(0, 12)}.jsonl`, import.meta.url));
  ...
  for (const direct of [false, true]) {
    ...
    const record = run(["bun", "run", CASE_RUNNER], {
    ...
    if (record.exitCode !== 0) throw new Error(`${transport} memory case failed:\n${record.stderr}`);
    const lines = record.stdout.trim().split(/\r?\n/).filter((line) => line.length > 0);
    if (lines.length !== 1) throw new Error(`${transport} memory case emitted ${lines.length} records`);
