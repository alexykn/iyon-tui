...
const CASE_RUNNER = fileURLToPath(new URL("./perf12_t15_multi_edit_case.ts", import.meta.url));
const EDIT_COUNTS = [2, 8, 32, 64] as const;
...
async function main(): Promise<void> {
  ...
  if (status !== "") throw new Error("authoritative multi-edit gate requires a clean committed checkout");
  ...
  const output = fileURLToPath(new URL(`./PERF-12-T15-multi-edit-${sourceSha.slice(0, 12)}.jsonl`, import.meta.url));
  await writeFile(output, "");

  for (const direct of [false, true]) {
    stage(direct);
    const transport = direct ? "feature_gated_direct_ffi" : "generated_safe_napi";
    for (const editCount of EDIT_COUNTS) {
      const result = run(["bun", "run", CASE_RUNNER], {
        T15_PROFILE: "authoritative",
        T15_TRANSPORT: transport,
        T15_EDIT_COUNT: String(editCount),
        T15_GIT_SHA: sourceSha,
        T15_RUSTC_VERSION: rustc,
        T15_TARGET: target,
        T15_NATIVE_SHA256: await addonSha(),
      });
      if (result.exitCode !== 0) throw new Error(`${transport} multi-edit/${editCount} failed:\n${result.stderr}`);
      const lines = result.stdout.trim().split(/\r?\n/).filter((line) => line.length > 0);
      if (lines.length !== 1) throw new Error(`${transport} multi-edit/${editCount} emitted ${lines.length} records`);
