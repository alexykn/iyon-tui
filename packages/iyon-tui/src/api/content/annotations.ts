...
export const TEXT_SOURCE_ANNOTATION_SCHEMA = Object.freeze([
	Object.freeze({
		kind: "tag",
		truncation: "clip",
		payload: "namespace\0name",
	}),
	Object.freeze({
		kind: "style",
		truncation: "clip",
		payload: "semantic-text-style-v1",
	}),
	Object.freeze({
		kind: "atomic",
		truncation: "drop",
...
export class Annotations {
	...
	constructor(
		...
		readonly properties: Readonly<Record<string, SemanticValue>> = {},
	) {}
	withTag(tag: SemanticTag): Annotations {
		return new Annotations([...this.tags, { ...tag }], this.properties);
	}
	withProperty(key: string, value: SemanticValue): Annotations {
		return new Annotations(this.tags, { ...this.properties, [key]: value });
	}
	containsTag(tag: SemanticTag): boolean {
		return this.tags.some(
			(item) => item.namespace === tag.namespace && item.name === tag.name,
		);
	}
}
