...
export class TextSelector {
	...
	static any(): TextSelector {
		return new TextSelector();
	}
	static role(role: TextRole): TextSelector {
		return TextSelector.any().role(role);
	}
	static part(part: TextPart): TextSelector {
		return TextSelector.any().part(part);
	}
	static annotation(namespace: string, name: string): TextSelector {
		return TextSelector.any().annotation(namespace, name);
	}
	static heading(): TextSelector {
		return TextSelector.role("heading");
	}
	static inlineCode(): TextSelector {
		return TextSelector.role("inlineCode");
	}
	static codeBlock(): TextSelector {
		return TextSelector.role("codeBlock");
	}
	...
	annotation(namespace: string, name: string): TextSelector {
		...
		return this.with({
			annotations: [...(this.value.annotations ?? []), { namespace, name }],
		});
	}
	...
	andFocused(): TextSelector {
		return this.with({ focused: true });
	}
	andFocusWithin(): TextSelector {
...
function validateTextRole(role: string): TextRole {
	if (!TEXT_ROLES.has(role as TextRole))
		throw new RangeError(`unknown text role ${JSON.stringify(role)}`);
	return role as TextRole;
}

function validateTextPart(part: string): TextPart {
	if (!TEXT_PARTS.has(part as TextPart))
		throw new RangeError(`unknown text part ${JSON.stringify(part)}`);
...
export function textStyle(
	value: StyleRef | StyleSpec | undefined,
): StyleRef | undefined {
