...
export class StyleSpec {
	...
	attribute(name: TextAttribute, enabled = true): StyleSpec {
		...
		if (typeof enabled !== "boolean")
			throw new TypeError("text attribute value must be boolean");
		return new StyleSpec({
			...this.value,
			attributes: { ...this.value.attributes, [name]: enabled },
		});
	}

	bold(): StyleSpec {
		return this.attribute("bold");
	}
	dim(): StyleSpec {
		return this.attribute("dim");
	}
	italic(): StyleSpec {
		return this.attribute("italic");
	}
	underline(): StyleSpec {
		return this.attribute("underline");
	}
	reversed(): StyleSpec {
		return this.attribute("reversed");
	}
	strikethrough(): StyleSpec {
		return this.attribute("strikethrough");
	}
	plain(): StyleSpec {
		return new StyleSpec({
			...this.value,
			attributes: Object.fromEntries(
				["bold", "dim", "italic", "underline", "reversed", "strikethrough"].map(
					(key) => [key, false],
				),
			),
		});
...
export class StyleRef {
	...
	static theme(
		key: string | ThemeKey,
		overrides: StyleSpec = new StyleSpec(),
	): StyleRef {
	...
	overrides(patch: StyleSpec): StyleRef {
		return new StyleRef(
			this.themeKey,
			new StyleSpec(mergeStyleValues(this.local.value, patch.value)),
		);
	}
}
...
export class StyleStateKey {
	...
	constructor(readonly value: string) {
		if (typeof value !== "string" || value.length === 0)
			throw new RangeError("style state key cannot be empty");
	}

	static from(value: string): StyleStateKey {
		return new StyleStateKey(value);
	}
}
...
export class StyleStateValue {
	...
	constructor(readonly value: string) {
		if (typeof value !== "string" || value.length === 0)
			throw new RangeError("style state value cannot be empty");
	}

	static from(value: string): StyleStateValue {
		return new StyleStateValue(value);
	}
}
...
export class StyleSelector {
	...
	static any(): StyleSelector {
		return new StyleSelector();
	}
	static focused(): StyleSelector {
		return StyleSelector.any().andFocused();
	}
	static focusWithin(): StyleSelector {
		return StyleSelector.any().andFocusWithin();
	}
	static state(
		key: string | StyleStateKey,
		value: string | StyleStateValue,
	): StyleSelector {
		return StyleSelector.any().andState(key, value);
	}

	andFocused(): StyleSelector {
		return this.with({ focused: true });
	}
	andFocusWithin(): StyleSelector {
		return this.with({ focusWithin: true });
	}
	andState(
		key: string | StyleStateKey,
		value: string | StyleStateValue,
		...
		return this.with({
			states: { ...(this.value.states ?? {}), [stateKey]: stateValue },
		});
	}
...
export function styleSelectorValue(
	selector: StyleSelector,
): StyleSelectorValue {
	return {
		...selector.value,
		...(selector.value.states === undefined
			? {}
			: { states: { ...selector.value.states } }),
	};
}
...
function stateKeyValue(key: string | StyleStateKey): string {
	const value =
		typeof key === "string" ? new StyleStateKey(key).value : key.value;
	if (typeof value !== "string" || value.length === 0)
		throw new RangeError("style state key cannot be empty");
	return value;
}
...
function stateValueValue(value: string | StyleStateValue): string {
	const result =
		typeof value === "string" ? new StyleStateValue(value).value : value.value;
