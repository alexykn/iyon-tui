...
import { type StyleSelector, StyleSpec, styleSelectorValue } from "./style.ts";
...
interface ThemeEntry<T> {
	...
	readonly variants: readonly {
		readonly selector: StyleSelectorValue;
		readonly value: T;
	}[];
}
...
export class Theme {
	...
	static new(): Theme {
		return new Theme(new Map(), new Map(), []);
	}
		...
		values.set(name, {
			...
			variants: [
				...(current?.variants ?? []).filter(
					(variant) => !styleSelectorsEqual(variant.selector, selectorValue),
				),
	...
	withTextStyle(selector: TextSelector, style: StyleSpec): Theme {
		return new Theme(this.styles, this.colors, [
			...this.textStyles,
			{ selector: selector.value, value: style.value },
		]);
	}
...
export function themeDefinitionFor(theme: Theme): ThemeDefinition {
	...
	return {
		styles: Object.fromEntries(
			[...data.styles].map(([name, entry]): [string, ThemeStyleEntry] => [
				name,
				{
...
/** Lowers a public Theme at the native host boundary. */
export function materializeTheme(theme: Theme): object {
	const definition = themeDefinitionFor(theme);
	const materializeColor = (color: ThemeColor): unknown => {
		switch (color.type) {
			case "default":
				return { type: "default" };
			case "named":
	...
	const materializeStyleColor = (color: ColorSpec): unknown => {
		switch (color.type) {
			case "theme":
				return `theme:${typeof color.key === "string" ? color.key : color.key.value}`;
			case "named":
				return color.value;
			case "indexed":
				return { type: "ansi", value: color.value };
	...
	const materializeStyle = (style: StyleSpecValue) => ({
		...(style.foreground === undefined
			? {}
			: { foreground: materializeStyleColor(style.foreground) }),
		...(style.background === undefined
			? {}
			: { background: materializeStyleColor(style.background) }),
		attributes: { ...style.attributes },
	});
	return {
		styles: Object.fromEntries(
			Object.entries(definition.styles).map(([name, entry]) => [
				name,
				{
					...(entry.base === undefined
						? {}
						: { base: materializeStyle(entry.base) }),
					variants: entry.variants.map((variant) => ({
						selector: variant.selector,
		...
		colors: Object.fromEntries(
			Object.entries(definition.colors).map(([name, entry]) => [
				name,
				{
					...(entry.base === undefined
						? {}
						: { base: materializeColor(entry.base) }),
					variants: entry.variants.map((variant) => ({
						selector: variant.selector,
						value: materializeColor(variant.value),
					})),
				},
			]),
		),
		textStyles: definition.textStyles.map((entry) => ({
			selector: entry.selector,
...
function styleSelectorsEqual(
	left: StyleSelectorValue,
	right: StyleSelectorValue,
): boolean {
	if (
		(left.focused === true) !== (right.focused === true) ||
		(left.focusWithin === true) !== (right.focusWithin === true)
	)
		return false;
