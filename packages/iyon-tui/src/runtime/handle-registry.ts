...
type HandleGlobals = typeof globalThis & {
	[HANDLE_ID_COUNTER]?: { next: number };
};
...
export function registerFrameworkHandle(
	...
	if (handleIdCounter.next > Number.MAX_SAFE_INTEGER)
		throw new Error("TUI framework handle identity exhausted");
	const id = handleIdCounter.next++ as HandleId;
	const resourceKind =
		kind === "component" || kind === "text-input" ? "component" : kind;
	try {
...
export function handleIdOf(handle: object): HandleId {
	...
	if (id === undefined)
		throw tuiError("disposed-handle", "framework handle has no local identity");
	return id;
}
