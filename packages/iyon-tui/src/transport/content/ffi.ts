...
interface ContentFfiSymbols {
	readonly iyon_tui_perf13_abi_metadata_v1: (
		out: Uint8Array | Uint32Array,
		outSize: number,
	) => number;
...
function assertU32(value: number, name: string): void {
	if (!Number.isSafeInteger(value) || value < 0 || value > MAX_U32) {
		throw contentError(
			"INVALID_ARGUMENT",
			`${name} must fit in an unsigned 32-bit lane`,
		);
	}
}
...
function splitU64(value: bigint | number, name: string): [number, number] {
	if (typeof value !== "bigint" && typeof value !== "number") {
		throw contentError(
			"INVALID_ARGUMENT",
			`${name} must be a safe integer or bigint`,
		);
	}
	...
	if (normalized < 0n || normalized > MAX_U64) {
		throw contentError(
			"INVALID_ARGUMENT",
			`${name} must fit in an unsigned 64-bit lane`,
		);
	}
...
function mutationResult(result: Uint32Array): MutationResult {
	if (result.length < CONTENT_ABI_MUTATION_RESULT_LANES) {
		throw contentError(
			"ABI_MISMATCH",
			"content mutation result buffer is too small",
		);
	}
	return {
...
function finishMutation(
	...
	if (name === undefined) {
		throw contentError(
			"ABI_MISMATCH",
			`unknown content ABI status ${status >>> 0}`,
		);
	}
	if (name !== "OK") {
		const message =
	...
	if (
		decoded.reserved0 !== 0 ||
		(decoded.flags & ~CONTENT_ABI_SCHEDULE_ENVIRONMENT_DRAIN) !== 0
	) {
		throw contentError(
			"ABI_MISMATCH",
			"content mutation result contains reserved bits",
		);
	}
	if ((decoded.flags & CONTENT_ABI_SCHEDULE_ENVIRONMENT_DRAIN) !== 0)
		requestWake();
	return {
...
function validateMetadata(metadata: Uint32Array): void {
	...
	const expect = (actual: number, expected: number, name: string): void => {
		if (actual !== expected)
			mismatches.push(`${name}=${actual}, expected ${expected}`);
	};
	...
	for (
		let index = 0;
		index < CONTENT_ABI_BUILD_FINGERPRINT.length;
		index += 1
	) {
		expect(
			metadata[14 + index]!,
			CONTENT_ABI_BUILD_FINGERPRINT[index]!,
			`buildFingerprint[${index}]`,
		);
		expect(
			metadata[22 + index]!,
			CONTENT_ABI_SCHEMA_FINGERPRINT[index]!,
			`schemaFingerprint[${index}]`,
		);
	}
	...
	if (mismatches.length > 0) {
		throw contentError(
			"ABI_MISMATCH",
			`content ABI metadata mismatch: ${mismatches.join("; ")}`,
		);
	}
}
...
function openSession(): ContentFfiSession {
	...
	const library = dlopen(artifact.absolutePath, {
		...
		iyon_tui_source_head_truncate_v1: {
			args: ["u32", "u32", "u32", "u32", "u32", "u32", "buffer"],
			returns: "u32",
		},
	});
	const metadata = new Uint32Array(CONTENT_ABI_METADATA_LANES);
	const status = library.symbols.iyon_tui_perf13_abi_metadata_v1(
		metadata,
		metadata.byteLength,
	);
	const name = contentStatusName(status);
	if (name !== "OK") {
		throw contentError(
			name ?? "ABI_MISMATCH",
			`content ABI metadata probe failed with ${status >>> 0}`,
		);
	}
...
function validateAnnotationObject(annotation: TextSourceAnnotation): void {
	if (typeof annotation !== "object" || annotation === null) {
		throw contentError(
			...
			"Source annotation must be an object",
		);
	}
	for (const key of Object.keys(annotation)) {
		if (
			...
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				`unknown Source annotation field ${JSON.stringify(key)}`,
			);
		}
	}
}
...
function annotationPayload(
	...
	if (kind === "style") {
		if (
			...
			annotation.namespace !== undefined ||
			annotation.name !== undefined
		) {
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				"style annotations use a semantic style, not tag names or opaque payload",
			);
		}
		if (annotation.style === undefined) {
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				"style annotations require a semantic style",
			);
		}
	...
	if (annotation.style !== undefined) {
		throw contentError(
			"INVALID_ANNOTATION_PAYLOAD",
			`${kind} annotations do not accept a semantic style`,
		);
	}
	if (kind === "tag") {
		if (annotation.payload !== undefined) {
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				"tag annotations use namespace and name, not payload",
			);
		}
		if (
			typeof annotation.namespace !== "string" ||
			typeof annotation.name !== "string"
		) {
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				"tag annotations require namespace and name",
			);
		}
		if (annotation.namespace.includes("\0") || annotation.name.includes("\0")) {
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				"tag annotation names must not contain NUL",
			);
		}
	...
	if (annotation.namespace !== undefined || annotation.name !== undefined) {
		throw contentError(
			"INVALID_ANNOTATION_PAYLOAD",
			`${kind} annotations do not accept tag names`,
		);
	}
	...
	if (!(annotation.payload instanceof Uint8Array)) {
		throw contentError(
			"INVALID_ANNOTATION_PAYLOAD",
			"annotation payload must be a Uint8Array",
		);
	}
...
function encodeSemanticStyle(
	...
	if (typeof style !== "object" || style === null) {
		throw contentError(
			"INVALID_ANNOTATION_PAYLOAD",
			"semantic style must be an object",
		);
	}
	for (const key of Object.keys(style)) {
		if (
			...
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				`unknown semantic style field ${JSON.stringify(key)}`,
			);
		}
	}
	...
	if (attributes !== undefined) {
		if (typeof attributes !== "object" || attributes === null) {
			throw contentError(
				"INVALID_ANNOTATION_PAYLOAD",
				"semantic text attributes must be an object",
			);
		}
		...
		for (const key of Object.keys(attributes)) {
			if (!(key in STYLE_ATTRIBUTE_BITS)) {
				throw contentError(
					"INVALID_ANNOTATION_PAYLOAD",
					`unknown semantic text attribute ${JSON.stringify(key)}`,
				);
			}
			...
			if (typeof value !== "boolean") {
				throw contentError(
					"INVALID_ANNOTATION_PAYLOAD",
					`semantic text attribute ${JSON.stringify(key)} must be boolean`,
				);
			}
			chunks[2] =
				(chunks[2]! | STYLE_ATTRIBUTE_BITS[key as TextAttribute]!) & 0xff;
			if (value)
				chunks[3] =
					(chunks[3]! | STYLE_ATTRIBUTE_BITS[key as TextAttribute]!) & 0xff;
		}
	}
	chunks[1] = flags;
...
function appendStyleString(
	...
	if (
		bytes.byteLength > 0xffff ||
		output.length + 2 + bytes.byteLength > maximum
	) {
		throw contentError(
			"PAYLOAD_TOO_LARGE",
			"semantic style string is too large",
		);
	}
	...
	switch (color.type) {
		...
		case "indexed":
			...
			if (color.value > 255)
				throw contentError(
					"INVALID_ANNOTATION_PAYLOAD",
					"semantic indexed color must fit in one byte",
				);
		...
		case "rgb":
			for (const [name, value] of [
				...
				if (!Number.isInteger(value) || value < 0 || value > 255) {
					throw contentError(
						"INVALID_ANNOTATION_PAYLOAD",
						`semantic ${name} channel must fit in one byte`,
					);
				}
			}
...
function validateStyleName(value: string, label: string): void {
	if (
		...
		throw contentError(
			"INVALID_ANNOTATION_PAYLOAD",
			`${label} must be non-empty and contain no whitespace or NUL`,
		);
	}
}

/** Decodes the fixed semantic-style payload exposed by Source snapshots. */
export function decodeSemanticStylePayload(
	payload: Uint8Array,
): SemanticTextStyle {
	if (payload.length < 4 || payload[0] !== STYLE_PAYLOAD_VERSION) {
		throw contentError(
			"ABI_MISMATCH",
			"semantic style payload version is unsupported",
		);
	}
	...
	const readString = (label: string): string => {
		if (offset + 2 > payload.length)
			throw contentError(
				"ABI_MISMATCH",
				`semantic style ${label} is truncated`,
			);
		...
		try {
			...
			throw contentError(
				"ABI_MISMATCH",
				`semantic style ${label} is not valid UTF-8`,
			);
		}
	};
	...
	const readColor = (label: string): ColorSpec => {
		...
		if (kind === 1) {
			...
			if (value === undefined)
				throw contentError(
					"ABI_MISMATCH",
					`semantic style ${label} ANSI color is invalid`,
				);
			return { type: "named", value };
		}
		if (kind === 2)
			return { type: "indexed", value: readByte(`${label} indexed color`) };
		if (kind === 3)
			return {
				type: "rgb",
		...
		if (kind === 4)
			return { type: "theme", key: readString(`${label} theme key`) };
		throw contentError(
			"ABI_MISMATCH",
			`semantic style ${label} color kind is invalid`,
		);
	};
	const role = flags & STYLE_FLAG_ROLE ? readString("role") : undefined;
	const foreground =
		flags & STYLE_FLAG_FOREGROUND ? readColor("foreground") : undefined;
	const background =
		flags & STYLE_FLAG_BACKGROUND ? readColor("background") : undefined;
	if (
		offset !== payload.length ||
		(!(flags & STYLE_FLAG_ATTRIBUTES) && (presence !== 0 || values !== 0))
	) {
		throw contentError(
			"ABI_MISMATCH",
			"semantic style payload has trailing or reserved data",
		);
	}
	const attributes: Partial<Record<TextAttribute, boolean>> = {};
	for (const [name, bit] of Object.entries(STYLE_ATTRIBUTE_BITS) as [
		TextAttribute,
		number,
	][]) {
...
function encodeAnnotations(
	...
	if (annotations.length > 16 * 1024) {
		throw contentError(
			"LIMIT_EXCEEDED",
			"Source annotation count exceeds the configured limit",
		);
	}
	const records = new Uint32Array(
		annotations.length * CONTENT_ABI_ANNOTATION_LANES,
	);
	...
	for (let index = 0; index < annotations.length; index += 1) {
		...
		if (typeof kind !== "string" || !Object.hasOwn(annotationKinds, kind)) {
			throw contentError(
				...
				`unknown Source annotation kind ${JSON.stringify(kind)}`,
			);
		}
		const annotationKind = kind as AnnotationKind;
		const start =
			annotation.startByte === undefined
				? annotationKind === "point"
					? bytes.byteLength
					: 0
				: annotation.startByte;
		const end =
			annotation.endByte === undefined
				? annotationKind === "point"
					? start
					: bytes.byteLength
				: annotation.endByte;
		...
		if (
			...
			throw contentError(
				"INVALID_RANGE",
				"Source annotation range is not a UTF-8 boundary range",
			);
		}
		if (annotationKind === "point" && start !== end) {
			throw contentError(
				"INVALID_RANGE",
				"point annotations must have an empty range",
			);
		}
		if (annotationKind !== "point" && start === end) {
			throw contentError(
				"INVALID_RANGE",
				"non-point annotations must cover text",
			);
		}
		...
		if (
			nextPayloadLength > MAX_ANNOTATION_PAYLOAD_BYTES ||
			nextPayloadLength > MAX_U32
		) {
			throw contentError(
				"PAYLOAD_TOO_LARGE",
				"Source annotation payload is too large",
...
function encodedText(text: string): Uint8Array {
	...
	if (
		bytes.byteLength > MAX_SOURCE_PAYLOAD_BYTES ||
		bytes.byteLength > MAX_U32
	) {
		throw contentError(
			"PAYLOAD_TOO_LARGE",
			"Source UTF-8 payload exceeds the ABI limit",
		);
	}
	return bytes;
}
...
function invokePayload(
	...
	const [environmentSlot, environmentGeneration, sourceSlot, sourceGeneration] =
...
export function contentFfiMetadata(): Readonly<{
	readonly artifactPath: string;
	readonly metadata: Uint32Array;
}> {
	...
	return {
		...
		metadata: current.metadata.slice(),
	};
}
