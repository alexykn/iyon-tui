...
export function resolveNativeArtifact(
	importerUrl: string | URL,
): NativeArtifactLocation {
	...
	const importerPath =
		typeof importerUrl === "string" && importerUrl.startsWith("file:")
	...
	if (candidate === undefined) {
		throw new Error(
			`iyon-tui-native Node addon is not staged for ${targetKey}; tried ${candidates.join(", ")}`,
		);
	}
	return {
