...
export class PreparedResourceLease {
	...
	get handleId(): HandleId {
		return this.record.handleId;
	}
	get kind(): NativeResourceKind {
		return this.record.kind;
	}
	get resource(): object {
		return this.keepAlive.resource;
	}
...
export class NativeResourceRegistry {
	...
	private readonly finalizer = new FinalizationRegistry<HandleId>(
		(handleId) => {
	...
	register(registration: ResourceRegistration): void {
		if (
			!isWeakReferenceable(registration.handle) ||
			!isWeakReferenceable(registration.resource)
		) {
			throw tuiError(
				"validation",
				"native resource registration requires object handles",
			);
		}
		if (
			!Number.isSafeInteger(registration.handleId) ||
			registration.handleId < 1
		) {
			throw tuiError(
				"validation",
				"framework handle identity must be a positive safe integer",
			);
		}
		if (
			this.records.has(registration.handleId) ||
			registration.handleId <= this.retiredThrough
		) {
			throw tuiError(
				"runtime",
				"framework handle identity is already retired or registered",
				{
		...
		if (
			this.handles.has(registration.handle) ||
			this.retiredHandles.has(registration.handle)
		) {
			throw tuiError(
				"runtime",
				"framework value already has a native resource",
			);
		}
		if (
			this.resources.has(registration.resource) ||
			this.retiredResources.has(registration.resource)
		) {
		...
		const acceptedNodeKinds =
			registration.acceptedNodeKinds === undefined
				? undefined
	...
	resourceForHandle(handle: object): object {
		...
		if (handleId === undefined)
			throw tuiError(
				"disposed-handle",
				"framework value has no live native resource",
			);
	...
	resourceForHandleId(
		...
		if (record === undefined || record.lifecycle !== "live") {
			throw tuiError(
				"disposed-handle",
				"framework handle has no live native resource",
				{ id: handleId },
			);
		}
		if (expectedKind !== undefined && record.kind !== expectedKind) {
			throw tuiError(
				"invalid-handle",
				`framework resource kind must be ${expectedKind}`,
				{
					id: handleId,
					expectedKind,
	...
	prepareResolve(
		...
		if (record === undefined || record.lifecycle === "disposed") {
			throw tuiError("disposed-handle", "framework attachment is disposed", {
				id: handleId,
			});
		}
		if (record.lifecycle === "disposing") {
			throw tuiError("invalid-handle", "framework attachment is disposing", {
				id: handleId,
			});
		}
		if (record.preparedLeases > 0) {
			throw tuiError(
				"validation",
				"framework attachment is already prepared for another candidate",
				{
					id: handleId,
				},
			);
		...
		if (handle === undefined || resource === undefined) {
			...
			throw tuiError(
				"disposed-handle",
				"framework attachment has no live owner/resource",
				{ id: handleId },
			);
		}
		const kindMatches =
		...
		if (!kindMatches) {
			throw tuiError(
				"invalid-handle",
				`framework attachment kind must be ${expectedKind}`,
				{
					id: handleId,
					expectedKind,
					actualKind: record.kind,
		...
		if (record.environment !== targetEnvironment) {
			throw tuiError(
				"invalid-handle",
				"framework attachment belongs to a different environment",
				{
					id: handleId,
				},
			);
		...
		if (record.owner?.host !== undefined && record.owner.host !== targetHost) {
			throw tuiError(
				"invalid-handle",
				"WRONG_HOST: framework attachment belongs to a different host",
				{
					id: handleId,
				},
			);
		...
		if (targetNodeKind !== undefined) {
			const validator = (
				resource as { readonly validateNodeKind?: (nodeKind: number) => void }
			).validateNodeKind;
	...
	release(handleId: HandleId): void {
		...
		if (
			...
			throw tuiError("invalid-handle", "resource still has active leases", {
				id: handleId,
			});
		}
		this.retireRecord(record);
	...
	private retireRecord(record: ResourceRecord): void {
		...
		if (record.handleId > this.retiredThrough)
			this.retiredThrough = record.handleId;
		const handle = record.handleRef.deref();
		if (handle !== undefined) {
		...
		if (
			resource !== undefined &&
			this.resources.get(resource) === record.handleId
		) {
